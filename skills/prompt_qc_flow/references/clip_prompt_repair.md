...
Additionally for role/place/wardrobe-prop drift:

5. Lock role ownership in conflict beats (protector, ledger holder, witness)
...
- For continuity-heavy scenes, lock the state that must not drift:
  ...
  - if a woman is present in Joseon common/noble dress, explicitly block male hat drift when needed (`No gat...`)
...
- Role lock (ledger ownership):
  - `Ledger is held only by Shim; Yeonhwa holds no ledger and no book.`
- Role lock (protector):
  - `Shim stands in front to shield Yeonhwa in a protective blocking stance.`
- Wardrobe lock (female hat drift):
  - `No gat, no male hat, and no male headgear on any female character.`
- Pregnancy lock:
  - `No baby in arms, no infant carried, unborn child only.`
- Place continuity lock:
  - `Setting continuity lock: keep this scene at the same courtyard/room/magistrate-office location.`
