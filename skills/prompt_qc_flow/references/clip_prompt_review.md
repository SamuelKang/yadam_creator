...
16. Detect and block role swap in conflict beats:
   - if script says Shim shields Yeonhwa, prompt must keep Shim as protector
   - if script says ledger/evidence is in Shim's hand, prompt must lock ownership and block Yeonhwa-ledger drift
17. Detect and block location drift in climax runs:
   - when adjacent scenes share a single place tag (`place_007` etc.), prompts must keep the same location family cues
18. Detect and block wardrobe/prop drift:
   - female `gat` is invalid unless explicitly required by script (normally forbidden in this project)
   - pregnancy beats must not drift into baby-in-arms imagery unless script explicitly advances to childbirth/post-birth
