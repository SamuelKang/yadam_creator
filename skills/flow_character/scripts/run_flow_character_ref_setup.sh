...
echo "[FLOW_CHARACTER] seed generation story=${STORY_ID} chars=${FLOW_SEED_CHAR_IDS}"
.venv/bin/python skills/flow_character/scripts/init_flow_character_seeds.py \
  --story-id "${STORY_ID}" \
  --char-ids "${FLOW_SEED_CHAR_IDS}" \
  "${SEED_ARGS[@]}"

echo "[FLOW_CHARACTER] collect reference points"
.venv/bin/python skills/flow_character/scripts/collect_flow_seed_refs.py \
  --story-id "${STORY_ID}"

echo "[FLOW_CHARACTER] done: work/${STORY_ID}/out/flow_seed_refs.json"
