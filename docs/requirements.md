...
캐릭터 이미지 생성 규격
	...
	•	조선 배경에서 생활 소도구가 중요한 캐릭터는 한국형 실루엣을 유지한다.
	•	예: 약초꾼/농부가 드는 낫은 서양식 큰 scythe가 아니라 한국형 `ㄱ`자 곡날 낫을 기본값으로 본다.
...
8.5 .vrew clip 자막 분할 옵션
	...
	•	자막 줄바꿈은 기본적으로 2줄 이하를 목표로 하며, 줄바꿈 위치는 단순 글자 수뿐 아니라 쉼표/마침표/호흡 단위도 함께 고려해야 한다.
	•	불가피하게 긴 문장은 .vrew clip 자체를 추가 분할해 3줄 노출을 줄인다.
...
매핑 규칙
	...
	•	scene.text는 .vrew clip으로 분할될 수 있으며(기본 30자), 분할 시 TTS 호흡 단위와 화면 자막 2줄 이하 가독성을 함께 고려한다.
	•	분할된 하위 clip들은 모두 동일 scene 이미지를 공유한다.
